...
if [[ "${EUID}" -ne 0 ]]; then
  echo "Ejecuta como root: sudo bash PATCH_UPDATE_MISMO_PUERTO.sh" >&2
  exit 1
fi

if [[ ! -d "$INSTALL_DIR" || ! -f "$CONF_FILE" ]]; then
  echo "No encontré instalación existente de svrcode-token-manager." >&2
  echo "Instala primero el token manager o usa install.sh." >&2
  exit 1
fi

cp "$INSTALL_DIR/api_server.py" "$INSTALL_DIR/api_server.py.bak.$(date +%s)" || true
cat > "$INSTALL_DIR/api_server.py" <<'PYAPI'
#!/usr/bin/env python3
...
add_env GITHUB_TOKEN COLOCA_AQUI_TU_TOKEN_DE_GITHUB
...
if ! grep -qE '^WEBHOOK_SECRET=' "$CONF_FILE"; then
  echo "WEBHOOK_SECRET=$(openssl rand -hex 32 2>/dev/null || date +%s%N)" >> "$CONF_FILE"
fi

mkdir -p /var/lib/svrcode-token/update-cache
chmod 700 /var/lib/svrcode-token/update-cache || true
...
echo "=========================================="
echo "PATCH APLICADO: token + update en el mismo puerto"
echo "Puerto: $PORT"
echo "Edita el token GitHub si falta:"
echo "nano $CONF_FILE"
echo "systemctl restart $SERVICE"
echo ""
echo "Prueba:"
echo "curl http://127.0.0.1:$PORT/health"
echo "curl http://127.0.0.1:$PORT/api/update/free"
echo "curl -H "X-App-Key: $APP_KEY" http://127.0.0.1:$PORT/api/admin/sync"
echo ""
echo "Webhook URL opcional:"
echo "http://TU_IP:$PORT/api/github/webhook"
echo "Webhook Secret:"
grep WEBHOOK_SECRET "$CONF_FILE" | cut -d= -f2-
echo "=========================================="
